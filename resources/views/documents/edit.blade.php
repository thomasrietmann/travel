...
@section('content')
    ...
    <form method="POST" action="{{ route('documents.update', $document) }}" enctype="multipart/form-data" class="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
        @csrf
        @method('PUT')
        @include('documents._form')
        <div class="mt-6 flex flex-col gap-3 sm:flex-row sm:justify-between">
            <button form="delete-document" class="rounded-md border border-red-200 px-4 py-2 text-sm font-medium text-red-700 hover:bg-red-50">Loeschen</button>
    ...
    <form id="delete-document" method="POST" action="{{ route('documents.destroy', $document) }}" onsubmit="return confirm('Dokument wirklich loeschen?')">
        @csrf
        @method('DELETE')
    </form>
@endsection
