...
@section('content')
    ...
    @unless ($public ?? false)
        <div class="mb-6 rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
            <p class="text-sm font-medium text-slate-950">Geschützter Countdown-Link</p>
            <p class="mt-1 text-sm text-slate-600">Dieser Link zeigt nur die Countdown-Übersicht ohne Login und ohne Zugriff auf weitere Bereiche.</p>
            <input readonly value="{{ $shareUrl }}" class="mt-3 w-full rounded-md border-slate-300 bg-slate-50 text-sm text-slate-700 shadow-sm">
        </div>
    @endunless
