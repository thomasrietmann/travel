...
@section('content')
    <div class="mb-6">
        ...
        <h1 class="mt-3 text-3xl font-semibold tracking-tight">Reise bearbeiten</h1>
    </div>

    <form method="POST" action="{{ route('trips.update', $trip) }}" class="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
        @csrf
        @method('PUT')
        @include('trips._form')
